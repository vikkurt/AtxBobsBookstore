    ...
    public static class ServicesSetup
    {
        ...
        private static string GetDatabaseConnectionString(ConfigurationManager configuration)
        {
            ...
            if (!string.IsNullOrEmpty(connString))
            {
                ...
                if (connString.Contains("Server=") || connString.Contains("Initial Catalog="))
                {
                    var sqlConnBuilder = new SqlConnectionStringBuilder(connString);
                    var npgsqlConnBuilder = new NpgsqlConnectionStringBuilder
                    {
                        Host = sqlConnBuilder.DataSource,
                        Database = sqlConnBuilder.InitialCatalog,
                        Username = sqlConnBuilder.UserID,
                        Password = sqlConnBuilder.Password
                    };
                    return npgsqlConnBuilder.ConnectionString;
                }
                return connString;
            }
